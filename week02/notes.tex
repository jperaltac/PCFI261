...
\titlegraphic{\hfill\includegraphics[height=1.7cm]{logo-unab.jpg}}
\date{Semana 02}
...
\begin{frame}{Entropía de Shannon: idea de fondo}
La entropía de Shannon mide cuán \textbf{repartida} o \textbf{incierta} está una distribución de probabilidad.

\begin{itemize}
    \item Si casi toda la probabilidad está en un solo estado, el sistema es más predecible y la entropía es baja.
    \item Si la probabilidad se distribuye entre muchos estados comparables, hay mayor incertidumbre y la entropía aumenta.
    \item No mide ``desorden'' en un sentido moral o visual: mide cuánta información falta antes de saber en qué estado está el sistema.
\end{itemize}

\begin{block}{Lectura física/computacional}
En esta semana la usaremos para resumir qué tan dispersos están los datos generados por una dinámica: una trayectoria concentrada en pocos valores produce menor entropía que una que visita muchas regiones del espacio de estados.
\end{block}
\end{frame}

\begin{frame}{Entropía de Shannon: fórmula y estimación}
...
\begin{itemize}
    \item Si $p_i=1/N$ para todo $i$, la distribución es uniforme y $S$ alcanza su valor máximo: $S_{\max}=\ln N$.
    \item Si algunos bins dominan y muchos tienen probabilidad casi nula, $S$ disminuye.
\end{itemize}

\vspace{0.15cm}
En simulaciones, normalmente no conocemos los $p_i$ exactos:
\begin{enumerate}
    \item generamos una muestra de datos $x_1,\dots,x_M$;
    \item dividimos el rango en bins;
    \item contamos frecuencias y las normalizamos para estimar $p_i$;
