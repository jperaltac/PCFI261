...
\centering
\resizebox{0.96\linewidth}{!}{\input{Figuras/week11_overview.tex}}
\end{columns}
\end{frame}
...
\centering
\resizebox{0.98\linewidth}{!}{\input{Figuras/dense_vs_cnn.tex}}
\end{columns}
\end{frame}
...
\begin{itemize}
    ...
    \item El pooling resume regiones y reduce sensibilidad a pequenos desplazamientos.
\end{itemize}
\begin{block}{Idea computacional}
La CNN no memoriza cada pixel de manera independiente: aprende patrones locales que pueden aparecer en muchas zonas.
\end{block}
\column{0.44\textwidth}
\centering
\resizebox{0.98\linewidth}{!}{\input{Figuras/cnn_pipeline.tex}}
\end{columns}
\end{frame}
...
\end{block}
\begin{alertblock}{Discusion}
Cuando falle, no basta decir que ``la red es mala''. Hay que mirar si el dato de entrada se parece o no a MNIST.
\end{alertblock}
\end{columns}
\end{frame}
